...
// Usage:
//   out\agent.exe --server http://127.0.0.1:8000 --token <AGENT_TOKEN>
//   out\agent.exe --server http://127.0.0.1:8000 --token <AGENT_TOKEN> \
...
class Agent
{
    ...
    static void Main(string[] args)
    {
        ...
        bool intervalGiven = false, jitterGiven = false, stateGiven = false, verboseGiven = false;
        for (int i = 0; i < args.Length; i++)
        {
            switch (args[i])
            {
                ...
                case "--interval": Interval = int.Parse(args[++i]); intervalGiven = true; break;
                case "--jitter": Jitter = int.Parse(args[++i]); jitterGiven = true; break;
                case "--state": StateFile = args[++i]; stateGiven = true; break;
