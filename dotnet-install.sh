...
# Persist the PATH entry so `dotnet` is available in new shells too. For the
# default install location this appends the standard line
# `export PATH="$HOME/.dotnet:$PATH"` to the user's shell profile.
persist_dotnet_path() {
    eval $invocation
    ...
    if [ -f "$dest" ] && grep -qsF "$line" "$dest"; then
        say_verbose "PATH export already present in $dest"
        return 0
    fi

    printf '\n# .NET SDK (installed by dotnet-install.sh)\n%s\n' "$line" >> "$dest"
    say "Added '$line' to $dest (start a new shell to take effect)."
}
...
if [ "$no_path" = false ]; then
    say "Adding to current process PATH: \`$bin_path\`."
